...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bottleneck(self.flat(self.avgpool(h)))
...
class Decoder(nn.Module):
    def __init__(self, lat_space):
        super().__init__()
        # self.bottleneck = nn.Linear(lat_space, 512)
        # self.unflat = nn.Unflatten(1, (512, 1, 1))
        # self.unpool = F.interpolate(scale_factor=7, mode='nearest')
        self.relu = nn.ReLU(True)
